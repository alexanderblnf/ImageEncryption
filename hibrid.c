...
void dither_mpi(Pixel *img, int x, int y) {
	...
	#pragma omp parallel for private(i, j, pos, newVal, errorR, errorG, errorB) schedule(dynamic, x)
 	for(i = 0; i < y; i++) {
		for(j = 0; j < x; j++) {
			pos = x * i + j;
...
void encrypt_mpi(char *inputFile, MPI_Datatype mpi_pixel, int nrOfThreads) {
	MPI_Status status;
	int rank, size, type;
...
void decrypt_mpi(char *shareFile, char *maskFile, MPI_Datatype mpi_pixel, int nrOfThreads) {
	MPI_Status status;
	int rank, size, type;
...
int main(int argc, char **argv) {
    ...
    if(strcmp(argv[1], "encrypt") == 0) {
    	...
    	encrypt_mpi(argv[2], mpi_pixel, nrOfThreads);
    } else if(strcmp(argv[1], "decrypt") == 0){
    	sscanf(argv[4], "%d", &nrOfThreads);
    	decrypt_mpi(argv[2], argv[3], mpi_pixel, nrOfThreads);
    }
